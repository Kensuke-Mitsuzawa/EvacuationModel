...
;;;;;                                                             ;;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; This code is modified by Kensuke Mitsuzawa
...
globals [        ; global variables
  ...
  max_ticks  ; max time to stop this simulation
...
to read-gis-files
  gis:load-coordinate-system word path_dataset_dir "/road_network/road_network.prj"                                          ; load the projection system - WGS84 / UTM (METER) for your specific area
  set shelter_locations gis:load-dataset word path_dataset_dir "/shelter_locations/shelter_locations.shp"                    ; read shelter locations
...
to load-tsunami
  ...
  file-close-all
  ifelse file-exists? word path_dataset_dir "/tsunami_inundation/details.txt" [
    file-open word path_dataset_dir "/tsunami_inundation/details.txt"
    ...
    foreach files [? ->
      ifelse file-exists? (word path_dataset_dir (word "/tsunami_inundation/" ? ".asc")) [
        let tsunami gis:load-dataset (word path_dataset_dir (word "/tsunami_inundation/" ? ".asc"))
  ...
  [
    ...
    let file_name word path_dataset_dir "/tsunami_inundation/boundaries.txt"
      ...
      foreach j [ k ->
        if length ( gis:location-of k ) = 2 [                          ; check if the vertex has both x and y
          ...
          create-residents 1 [                                         ; create the agent
            ...
            ifelse x = [xcor] of init_dest and y = [ycor] of init_dest [
              set evacuated? true
            ][
...
to go
  ...
  ; A procedure, immediately evacuated.
  ask residents with [evacuated?][
    hatch-pedestrians 1 [
          set size 2
          set shape "dot"
          set current_int myself ; myself = current_int of the resident
          set speed 0.0          ; the speed of the resident is passed on to the pedestrian
          set evacuated? true   ; initialized as not evacuated, will be checked immediately after being born
  ...
  ask residents with [not evacuated? and not reached?][
  ...
  ask pedestrians with [not moving? and not evacuated? and not empty? path and not dead?][
